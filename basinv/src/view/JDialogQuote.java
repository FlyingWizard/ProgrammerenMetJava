...
import utilities.GenericTableModel;
import utilities.NumberEnum;
import view.JDialogInvoice.MyTableModel;
...
public class JDialogQuote extends JDialog {
	...
	private JDialogQuote(JFrame frame, boolean modal,
		...
		if (RDBConnection.getProps().getProperty(Constants.QUOTE_DIALOG)
	...
	private void quoteDetails() {
		{// quote Details
			...
			jInternalFrameQuoteDetail.setTitle(Constants.DETAILS);
			{
				...
				jPanelQuoteDetail.setPreferredSize(new java.awt.Dimension(790, 46));
	...
	private JButton getJButtonQuoteDetailNew() {
		if (jButtonQuoteDetailNew == null) {
			...
			jButtonQuoteDetailNew.setPreferredSize(new java.awt.Dimension(81, 26));
		}
					...
					QuoteDetail detail = QuoteDetailController
							.getOneQuoteDetail(quote.getIdQuote(),
									(Integer) jTableQuoteDetails.getModel()
											.getValueAt(row, 0));
					if ((detail != null) && (detail.getQtePrice().doubleValue() != Figures.ZERO)) {
	...
	private void initColumnSizes(JTable table) {
		...
		Object[] longValues = { new Integer(99), Constants.GENERAL_PURPOSE,
				new BigDecimal(1000000), new BigDecimal(1000000), Constants.UNIT,
